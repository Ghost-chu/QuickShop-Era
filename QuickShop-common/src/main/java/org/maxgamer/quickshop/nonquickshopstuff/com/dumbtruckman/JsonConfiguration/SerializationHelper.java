...
public class SerializationHelper {
     ...
     * ConfigurationSerialization#deserializeObject(Map)}. Basically this means it will
    ...
    /**
     * Takes a Map and parses through the values, to ensure that, before saving, all objects are as
     * appropriate as possible for storage in most data formats.
     *
     * <p>Specifically it does the following: for Map: calls this method recursively on the Map before
     * putting it in the returned Map. for List: calls {@link #buildList(Collection)} which
     * functions similar to this method. for ConfigurationSection: gets the values as a map and calls
     * this method recursively on the Map before putting it in the returned Map. for
     * ConfigurationSerializable: add the {@link ConfigurationSerialization#SERIALIZED_TYPE_KEY} to a
     * new Map along with the Map given by {@link
     * org.bukkit.configuration.serialization.ConfigurationSerializable#serialize()} and calls this
     * method recursively on the new Map before putting it in the returned Map. for Everything else:
     * stores it as is in the returned Map.
     */
    @NotNull
    ...
    /**
     * Takes a Collection and parses through the values, to ensure that, before saving, all objects
     * are as appropriate as possible for storage in most data formats.
     *
     * <p>Specifically it does the following: for Map: calls {@link #buildMap(Map)} on the
     * Map before adding to the returned list. for List: calls this method recursively on the List.
     * for ConfigurationSection: gets the values as a map and calls {@link #buildMap(Map)}
     * on the Map before adding to the returned list. for ConfigurationSerializable: add the {@link
     * ConfigurationSerialization#SERIALIZED_TYPE_KEY} to a new Map along with the Map given by {@link
     * org.bukkit.configuration.serialization.ConfigurationSerializable#serialize()} and calls {@link
     * #buildMap(Map)} on the new Map before adding to the returned list. for Everything
    ...
    /**
     * Parses through the input list to deal with serialized objects a la {@link
     * ConfigurationSerializable}.
     *
     * <p>Functions similarly to {@link #deserialize(Map)} but only for detecting lists
